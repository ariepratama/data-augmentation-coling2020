...
def generate_sentences_by_grammar(sentence: Sentence,
    ...
    selector = RandomWalkSelector(num_generated_samples)
    ...
    for generation_idx, (target_parent, target_child_idx) in enumerate(
            selector.select(sentence_tree, num_generated_samples)):
        logging.info(target_parent)
        mutated_parent, _, selected_corpus_indexes = mutator.transform(target_parent, target_child_idx,
                                                                       random_seed=random_state)
        tok2tag = generate_tok2label(idx2sentence, selected_corpus_indexes)
...
def get_start_end_of_span_rec(explored_node, target_node, leaves):
    ...
    if explored_node == target_node:
        ...
        if type(explored_node) == Tree:
            selected_end_span_idx = selected_start_span_idx + max(len(explored_node.leaves()), 1)
...
def generate_tok2label(idx2sentence, selected_corpus_indexes):
    tok2label = {}
    for corpus_idx in selected_corpus_indexes:
        corpus_sentence = idx2sentence[corpus_idx]

        for token in corpus_sentence:
            if token.text not in tok2label:
                tok2label[token.text] = token.get_label("gold")
            else:
...
def to_sentence(original_sentence, n_generated_sentence, leaves,
    ...
    for token_idx, leave in enumerate(leaves):
        ...
        if start_mutated_idx <= token_idx < end_mutated_idx:
            label = tok2tag[text]
        elif -1 < bm_start <= token_idx < bm_end:
            label = original_sentence.get_token(token_idx).get_label("gold")
        elif -1 < am_start <= token_idx:
            new_token_idx = am_start + token_idx - end_mutated_idx
            logging.info(
                f"getting label to normalized new_token_idx={new_token_idx}, am_start={am_start}, token_idx={token_idx}, end_mutated_idx={end_mutated_idx}"
            )
...
def find_before_and_after_mutation_idx(original_sentence, leaves, start_mutated_idx, end_mutated_idx) -> Tuple[
    ...
    if start_mutated_idx == 0:
        return -1, -1, end_mutated_idx, len(leaves)

    # mutation happened from middle to the end of sentence
    if end_mutated_idx + 1 == len(leaves):
        return 0, start_mutated_idx, -1, -1

    bm_start, bm_end = 0, 0
    bm_end = start_mutated_idx
